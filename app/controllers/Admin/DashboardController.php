...
class DashboardController extends \BaseController {
	...
	public static function index() {
		...
		$data['total_credits']  = \Sale::where('sale_type','=', 'CREDIT')->sum('total_amount');
		$data['total_expense']  = \Expense::sum('total_amount');
		$data['total_sales'] = \Sale::where('sale_type', 'SALE')->sum('total_amount'); //select(\DB::raw('TRUNCATE(SUM(total_amount - (supplier_price * quantity)), 2) as total_sale'))->pluck('total_sale');
