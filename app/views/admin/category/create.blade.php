    ...
    @include ('admin._partials.breadcrumbs')
	  <section class="panel">
		...
		<div class="panel-body">
		  	<form action="{{ route('admin_categories.store') }}"  class="form-horizontal tasi-form" method="POST">
				  ...
				  <div class="col-sm-10">
				      @if ($brands)
				  			@foreach ($brands as $brand)
				  				<input type="checkbox" name="brand[]" value="{{ $brand->brand_id }}" {{{ in_array($brand->brand_id, Input::old('brand', [])) ? 'checked' : '' }}}> {{ $brand->name }}
				  			@endforeach
				  		@endif
				  </div>
				</div>
